...
io.on("connection", (socket) => {
    ...
    console.log(`[SOCKET] Connection attempt. Socket ID: ${socket.id}, User ID: ${userId}`);

    // We could also verify the token here if we passed it in socket.handshake.auth.token
    if (userId) {
        userSocketMap[userId] = socket.id;
        console.log(`[SOCKET] User mapped: ${userId} -> ${socket.id}`);
        console.log(`[SOCKET] Online Users: ${Object.keys(userSocketMap).length}`);
    }
